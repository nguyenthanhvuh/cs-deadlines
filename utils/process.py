...
with open("../_data/conferences.yml", 'r') as stream:
    try:
        ...
        # just sort:
        conf.sort(key=lambda x: pytz.utc.normalize(datetime.datetime.strptime(
            x['deadline'], dateformat).replace(tzinfo=pytz.timezone(x['timezone']))))
        print("Date Sorting:")
        for q in conf+tba:
            print(q["deadline"], " - ", q["name"])
        print("\n\n")
        conf.sort(key=lambda x: pytz.utc.normalize(datetime.datetime.strptime(x['deadline'], dateformat).replace(
            tzinfo=pytz.timezone(x['timezone']))).strftime(dateformat) < right_now)
...
with open('sorted_data.yml', 'r') as stream:
    try:
        ...
        for q in conf:
            ...
            try:
                start_date = dates.strip().split(" ")[0].strip(
                )+" "+dates.split("-")[1].strip()+" "+year.strip()
            except IndexError:
                month, day = dates.strip().split(" ")
                start_date = month.strip()+" "+day.strip()+" "+year.strip()
            try:
                datetime.datetime.strptime(
                    start_date, "%B %d %Y").strftime(dateformat)
            except ValueError:
